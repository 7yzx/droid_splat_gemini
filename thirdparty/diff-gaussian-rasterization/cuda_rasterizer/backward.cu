	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if constexpr (DEPTH)
			{
				...
				// atomicAdd(&dL_dray_planes[global_id].x, dL_dt * d.x / focal_x); //  this is in in rade gs 
				// atomicAdd(&dL_dray_planes[global_id].y, dL_dt * d.y / focal_y); // but done below 
			}
	...
	float2* dL_dray_planes)
{
    renderCUDA<NUM_CHANNELS, true> <<<grid, block>>> ( \
		...
		accum_depth, \
        n_contrib, dL_dpixels, dL_dpixel_depth, dL_pixel_mdepth, \
